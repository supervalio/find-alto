...
import { CountryPhotoField } from "@/app/admin/countries/photo-field";
...
export default async function CountriesPage({
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold tracking-tight mb-6">Страны</h1>
      ...
      >
        <summary className="cursor-pointer text-sm font-medium text-zinc-900 select-none">
          {editing ? "✏️ Редактировать страну" : "+ Добавить страну"}
        </summary>
        <form action={createCountry} className="mt-4 space-y-4">
          <div>
      ...
      {editing && (
        <div className="mb-8 rounded-xl border border-zinc-200 bg-white p-6">
          ...
          <form action={updateCountry} className="space-y-4">
            ...
            <CountryPhotoField defaultValue={editing.image ?? ""} />
            <button
              type="submit"
              className="rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white hover:bg-zinc-800 transition-colors"
            >
